...
public function getTable_gamma() {
  return DB_TABLE_PREFIX.'t_item_gamma';
}
...
public function import($file) {
  ...
  if(!$this->dao->importSQL($sql) ){
    throw new Exception("Error importSQL::ModelGAM<br>" . $file . "<br>" . $this->dao->getErrorLevel() . " - " . $this->dao->getErrorDesc() );
  }
}


public function install() {
  $this->import('oc-content/themes/gamma/model/struct.sql');
}
...
public function getExtra($item_id) {
  ...
  $this->dao->from($this->getTable_gamma() . ' as a');
  //$this->dao->join($this->getTable_gamma_locale() . ' as al', '(al.fk_i_item_id = a.fk_i_item_id AND al.fk_c_locale_code = "' . osc_current_user_locale() . '")', 'LEFT OUTER');
...
public function updateExtra($params) {
  return $this->dao->replace($this->getTable_gamma(), $params);
}

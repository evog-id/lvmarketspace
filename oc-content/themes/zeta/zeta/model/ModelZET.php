...
public function findCategories($pattern, $limit = 6, $parent_cat_id = NULL) {
  $this->dao->select('c.pk_i_id, c.fk_i_parent_id, cd.s_name, pd.s_name as s_name_parent');
  $this->dao->from($this->getTable_category() . ' as c');
  $this->dao->join($this->getTable_category_description() . ' as cd', '(c.pk_i_id = cd.fk_i_category_id AND cd.fk_c_locale_code = "' . osc_current_user_locale() . '")', 'INNER');
  $this->dao->join($this->getTable_category() . ' as p', 'c.fk_i_parent_id = p.pk_i_id', 'LEFT OUTER');
  $this->dao->join($this->getTable_category_description() . ' as pd', '(c.fk_i_parent_id = pd.fk_i_category_id AND pd.fk_c_locale_code = "' . osc_current_user_locale() . '")', 'LEFT OUTER');

  $this->dao->where('c.b_enabled', 1);
  
  if($pattern != '') {
    $this->dao->where(sprintf('(cd.s_name like "%%%s%%" OR COALESCE(pd.s_name,"") like "%%%s%%")', osc_esc_html($pattern), osc_esc_html($pattern)));
  }
  
  if((int)$parent_cat_id > 0) {
    $this->dao->where(sprintf('(c.pk_i_id = %d OR COALESCE(c.fk_i_parent_id,0) = %d OR COALESCE(p.fk_i_parent_id,0) = %d)', osc_esc_html((int)$parent_cat_id), osc_esc_html((int)$parent_cat_id), osc_esc_html((int)$parent_cat_id)));
  }
...
public function findCities($pattern, $limit = 6) {
  ...
  if($pattern != '') {
    if(osc_get_current_user_locations_native() == 1) {
      $this->dao->where(sprintf('(c.s_name like "%s%%" OR COALESCE(c.s_name_native,"") like "%s%%")', osc_esc_html($pattern), osc_esc_html($pattern)));
    } else {
...
public function findRegions($pattern, $limit = 6) {
  ...
  if($pattern != '') {
    if(osc_get_current_user_locations_native() == 1) {
      $this->dao->where(sprintf('(s_name like "%s%%" OR COALESCE(s_name_native,"") like "%s%%")', osc_esc_html($pattern), osc_esc_html($pattern)));
    } else {

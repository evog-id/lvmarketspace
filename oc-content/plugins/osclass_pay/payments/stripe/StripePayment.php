...
class StripePayment {
  ...
  public static function processPayment() {
    ...
    if(!isset($pending['s_transaction_id']) || trim($pending['s_transaction_id']) == '') {
    ...
    if(substr($pending['s_transaction_id'], 0, 3) == 'pi_') {
      $intent = \Stripe\PaymentIntent::retrieve($pending['s_transaction_id']);
      $paid = ($intent->status == 'succeeded' ? true : false);
      $transaction_id = $pending['s_transaction_id'];
      
    } else if(substr($pending['s_transaction_id'], 0, 3) == 'cs_') {
      $session = \Stripe\Checkout\Session::retrieve($pending['s_transaction_id']);
      $paid = ($session->payment_status == 'paid' ? true : false);   // or $session->status == 'complete'

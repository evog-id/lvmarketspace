...
function seller_post($user_id = '') {
  ...
  if(osc_get_preference('rewriteEnabled', 'osclass') <> 1) {
    $url = osc_base_url() . 'index.php?page=search&seller_post=' . $user_id;
  } else {
    //$url = osc_base_url() . 'search/seller_post,' . $user_id . '/iPage';
    $url = osc_search_url(array('page' => 'search', 'seller_post' => $user_id, 'iPage' => ''));
  }
  ...
  if ( $user_id <> 0 ) {
    ...
    echo __('Dashboard', 'list_seller_items') . ' (';
    echo $user['i_items'] == 1 ? sprintf(__('%d item', 'list_seller_items'), 1) : sprintf(__('%d items', 'list_seller_items'), $user['i_items']) . ')';
    echo '</a>';
  }
}
